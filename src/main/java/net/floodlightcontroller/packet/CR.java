 ...
 */
public class CR extends BasePacket {
    ...
    static {
         ...
         */
        CR.decodeMap.put(DHCP_CLIENT_PORT, DHCP.class);
        CR.decodeMap.put(DHCP_SERVER_PORT, DHCP.class);
     ...
     */
    public CR setSourcePort(TransportPort sourcePort) {
     ...
     * @param sourcePort the sourcePort to set
     */
    public CR setSourcePort(short sourcePort) {
    ...
    public int getCID() {
    	int CID;
    	CID=Integer.parseInt(this.getDestinationPort().toString());
    	CID=CID<<16;
    	CID=Integer.parseInt(this.getSourcePort().toString())& 0xffff;
        return CID;
    }
     ...
     * @param sourcePort the sourcePort to set
     */
    public CR setCID(int CID) {
       // int CID;// = CID;
        this.setSourcePort((short)CID);
        this.setDestinationPort((short)(CID >> 16));
        return this;
    }
    ...
    /*
    public CR setCID(int CID) {
        this.sourcePort = TransportPort.of(CID);
        return this;
    }*/
    /////////////////////////////////////////////
    /**
     ...
     */
    public CR setDestinationPort(TransportPort destinationPort) {
     ...
     * @param destinationPort the destinationPort to set
     */
    public CR setDestinationPort(short destinationPort) {
     ...
     */
    public CR setChecksum(short checksum) {
    ...
    public byte[] serialize() {
        ...
        //bb.putInt((int)this.getCID());   //CR packet CID 32 bit
        ...
        if (this.parent != null && this.parent instanceof IPv4)
            ((IPv4)this.parent).setProtocol(IpProtocol.of((short)250));//protocol number in IP packet
    ...
    public IPacket deserialize(byte[] data, int offset, int length)
        ...
        if (CR.decodeMap.containsKey(this.destinationPort)) {
            try {
                this.payload = CR.decodeMap.get(this.destinationPort).getConstructor().newInstance();
            } catch (Exception e) {
                throw new RuntimeException("Failure instantiating class", e);
            }
        } else if (CR.decodeMap.containsKey(this.sourcePort)) {
            try {
                this.payload = CR.decodeMap.get(this.sourcePort).getConstructor().newInstance();
            } catch (Exception e) {
                throw new RuntimeException("Failure instantiating class", e);
            }
